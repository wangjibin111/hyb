...
function showMsg(message) {
  ...
  function addPoints() {
    ...
    lines.forEach(line => {
      ...
      // 可添加数据格式验证代码
      fetch('http://https://hyb-78jysttce-jibin-wangs-projects.vercel.app:3000/addPoints', {
        method: 'POST',
  ...
  function subPoints() {
    ...
    lines.forEach(line => {
      if (!line.trim()) return;
      const [id, score] = line.trim().split(/\s+/);
      fetch('http://https://hyb-78jysttce-jibin-wangs-projects.vercel.app:3000/subPoints', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          student_id: parseInt(id),
  ...
  function queryPoints() {
    fetch('http://https://hyb-78jysttce-jibin-wangs-projects.vercel.app:3000/query')
      .then(res => res.json())
      .then(data => {
        if (data.success) {
  ...
  function clearPoints() {
    fetch('http://https://hyb-78jysttce-jibin-wangs-projects.vercel.app:3000/clear', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' }
    })
    .then(res => res.json())
